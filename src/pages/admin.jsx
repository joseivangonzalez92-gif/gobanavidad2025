      ...
      <div className="p-4 border border-yellow-200 rounded-lg bg-yellow-50">
        ...
        >
          🔍 Ver Funciones Time
            ...
            {/* 🚨 NUEVA SECCIÓN: CONTROL TABLERO BINGO */}
        <div className="mt-8 p-6 bg-red-50 border-2 border-red-300 rounded-xl">
          <h3 className="text-2xl font-bold mb-4 text-red-800">🎯 Control del Tablero de Bingo Compartido</h3>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <button
                onClick={async () => {
                  if (confirm('¿Estás seguro de reiniciar el tablero? Esto creará un nuevo tablero vacío.')) {
                    try {
                      setLoading(true);
                      await gobaService.inicializarTableroCompartido();
                      alert('✅ Tablero reiniciado correctamente');
                    } catch (error) {
                      console.error('❌ Error reiniciando tablero:', error);
                      alert('❌ Error reiniciando tablero: ' + error.message);
                    } finally {
                      setLoading(false);
                    }
                  }
                }}
                className="w-full bg-red-600 hover:bg-red-700 text-white py-4 rounded-xl font-bold text-lg transition-all transform hover:scale-105 shadow-xl flex items-center justify-center gap-3"
              >
                <span className="text-2xl">🔄</span>
                REINICIAR TABLERO BINGO
              </button>
              
              <div className="bg-white p-4 rounded-lg border border-red-200">
                <h4 className="font-semibold text-red-700 mb-2">⚠️ ¿Qué hace este botón?</h4>
                <ul className="text-sm text-gray-700 space-y-1">
                  <li>• Crea un nuevo tablero de 5x5 con 25 celdas</li>
                  <li>• Restablece todas las celdas como "disponibles"</li>
                  <li>• Elimina cualquier selección anterior</li>
                  <li>• Útil para empezar una nueva ronda de juego</li>
                </ul>
              </div>
            </div>
            
            <div className="bg-white p-4 rounded-lg border border-blue-200">
              <h4 className="font-semibold text-blue-700 mb-2">📊 Información del Tablero</h4>
              <div className="text-sm text-gray-700 space-y-2">
                <p>• <strong>Estado actual:</strong> {gobaService.tableroService?.estado || 'Desconocido'}</p>
                <p>• <strong>Total celdas:</strong> 25 (5x5)</p>
                <p>• <strong>Celdas seleccionadas:</strong> 0</p>
              ...
              <button
                onClick={async () => {
                  try {
                    setLoading(true);
                    const estado = await gobaService.tableroService.obtenerEstadoTablero();
                    alert(`🎯 Estado del Tablero:\n\n• ID: ${estado.id}\n• Creado: ${new Date(estado.createdAt).toLocaleString()}\n• Celdas totales: ${estado.totalCeldas}\n• Celdas seleccionadas: ${estado.celdasSeleccionadas || 0}`);
                  } catch (error) {
